...
def test_010_recipe_insert(app):
...
def test_020_recipe_delete(app):
    with app.app_context():
        name, error = recipes.delete(db.load(), 1)
    assert error is None and name == "test recipe"


def test_030_recipe_select(app):
    with app.app_context():
        recipe, error = recipes.select(db.load(), 1)
    assert error is None and recipe["name"] == "test recipe"


def test_040_recipes_from_user(app):
    with app.app_context():
        rows, error = recipes.from_user(db.load(), 1)
    assert error is None and len(rows) == 1 and rows[0]["rowid"] == 1


def test_050_recipes_dump(app):
    with app.app_context():
